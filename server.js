...
// ポモロードのページを表示
fastify.get("/pomodoro", async (request, reply) => {
  // ポモロードセット数をデータベースから取得
  const pomodoroCount = await db.getPomodoroCount();

  // クエリパラメータで指定された「command=kusoapuri」の場合
  if (request.query.command === 'kusoapuri') {
    // ポモロードタイマーのページに遷移
    return reply.view("/src/pages/pomodoro.hbs", { setCount: pomodoroCount });
  }
...
// ポモロードのセット数を更新
fastify.post("/pomodoro", async (request, reply) => {
  // セット数をインクリメント
  await db.incrementPomodoroCount();

  // 再度ポモロードページにリダイレクト
  return reply.redirect("/pomodoro");
});
